...
```"""
    ...
    def extract_code_blocks(self, response: str) -> Dict[str, str]:
        ...
        # Pattern to match file headers and code blocks with optional leading whitespace
        # Handle cases with or without language specification
        pattern = r'\s*### File: ([^\n]+)\n\s*```(?:\w+)?\n(.*?)\n\s*```'
...
class AgentEngine:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        # Load callback context if provided
        self.callback_context = None
        self.callback_func = None
        if args.callback_file and os.path.exists(args.callback_file):
            try:
                with open(args.callback_file, 'r') as f:
                    callback_data = json.load(f)
                    self.callback_context = callback_data.get('context')
                    if callback_data.get('type') == 'slack':
                        self.callback_func = self._slack_callback_wrapper
            except Exception as e:
                logger.warning(f"Failed to load callback context: {e}")
    ...
    def execute_workflow(self) -> None:
        ...
        try:
            ...
            self._send_callback('started', f'Starting workflow for issue {self.args.issue_id}')
            
            # Step 1: Checkout branch
            if not self.git_manager.checkout_branch(self.branch_name):
                self._send_callback('failed', 'Failed to checkout branch')
            ...
            self._send_callback('started', 'Generating code with Claude AI')
            ...
            if not claude_response:
                self._send_callback('failed', 'Failed to generate code with Claude AI')
            ...
            if self.args.test_command:
                self._send_callback('testing', 'Running tests')
                test_success, test_output = self.test_executor.run_tests(self.args.test_command)
                
                if test_success:
                    # Step 6: Commit and push
                    self._send_callback('committing', 'Tests passed, committing changes')
                    ...
                    if self.git_manager.stage_and_commit(commit_message, written_files):
                        if self.git_manager.push_to_remote(self.branch_name):
                            self._send_callback('success', f'Successfully completed task for issue {self.args.issue_id}')
                            self._report_success(claude_response, written_files, test_output)
                        else:
                            self._send_callback('failed', 'Failed to push changes to remote')
                            self._report_failure("Failed to push changes to remote")
                    else:
                        self._send_callback('failed', 'Failed to commit changes')
                        self._report_failure("Failed to commit changes")
                else:
                    self._send_callback('failed', 'Tests failed')
...
*Generated by Multi-Agent TDD System*"""
    ...
    def _slack_callback_wrapper(self, status: str, message: str):
        """Wrapper for Slack callback functionality."""
        if not self.callback_context:
            return
        
        try:
            # Import here to avoid circular imports
            from presentation.api.slack_bot import _slack_progress_callback
            _slack_progress_callback(self.callback_context, status, message)
        except Exception as e:
            logger.error(f"Failed to send Slack callback: {e}")
    
    def _send_callback(self, status: str, message: str):
        """Send progress callback if available."""
        if self.callback_func:
            try:
                self.callback_func(status, message)
            except Exception as e:
                logger.error(f"Callback error: {e}")


class AgentDispatcher:
    """Dispatcher for executing agent tasks with optional callback support."""
    ...
    def dispatch_agent_task_with_callback(
        ...
        """
        try:
            logger.info(f"Dispatching agent task for issue {issue_id}")
            
            # Notify callback of task start
            if callback_func and callback_context:
                callback_func(callback_context, 'started', f'Starting agent task for issue {issue_id}')
            ...
            # Execute the agent engine as subprocess
            ...
            # Don't wait for completion in webhook context
            logger.info(f"Agent task dispatched for issue {issue_id}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to dispatch agent task: {e}")
            if callback_func and callback_context:
                callback_func(callback_context, 'failed', f'Failed to start agent task: {str(e)}')
            return False
        finally:
            # Clean up callback file
            if callback_file and os.path.exists(callback_file.name):
                try:
                    os.unlink(callback_file.name)
                except Exception as e:
                    logger.error(f"Failed to clean up callback file: {e}")
...
def main():
    ...
    parser.add_argument(
        "--callback-file",
        help="JSON file containing callback context for progress updates"
    )
